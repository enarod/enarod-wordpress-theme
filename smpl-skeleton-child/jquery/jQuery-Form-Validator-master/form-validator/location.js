(function ($) {
	...
	var _toLowerCaseCollection = function(collection) {
		var result = [];

		for (var item in collection) {
			result.push(collection[item].toLowerCase());
		}
	...
	$.formUtils.addValidator(
		{
			name: "ukrainiancity",
			validatorFunction: function (str) {
				var isInArray = $.inArray(str.toLowerCase(), _toLowerCaseCollection(this.cities)) > -1;
				return isInArray;
			},
			cities: ["Вінниця", "Дніпропетровськ", "Донецьк", "Житомир", "Запоріжжя", "Івано-Франківськ", "Київ", "Кіровоград", "Луганськ", "Луцьк", "Львів", "Миколаїв", "Одеса", "Полтава", "Рівне", "Сімферополь", "Суми", "Тернопіль", "Ужгород", "Харків", "Херсон", "Хмельницький", "Черкаси", "Чернівці", "Чернігів", "Інше"],
			errorMessage: "",
			errorMessageKey: "badCustomVal"
		});
	...
	$.fn.suggestUkrainianCity = function (settings) {
		var cities = _makeSortedList($.formUtils.validators.validate_ukrainiancity.cities);
		return $.formUtils.suggest(this, cities, settings);
	};
})(jQuery);